...
- **Proximity View unassigned file grouping**: Instead of a flat global `community_unassigned` bucket, files without community clusters are grouped dynamically by their parent directory paths in the Proximity view.
...
- **Proximity unassigned cluster labels**: Labeled unassigned clusters dynamically as `Unassigned in <directory_path> (<count> files)` with correct drill-down capabilities.
