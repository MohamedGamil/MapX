...
(call
  ...
  (arguments (alias) @symbol.name)) @symbol.kind_module

; Public function definitions: def foo(args) do ... end
(call
  target: (identifier) @_def
  (#eq? @_def "def")
  (arguments (call target: (identifier) @symbol.name))) @symbol.kind_function
...
(call
  ...
  (arguments (alias) @symbol.name)) @symbol.kind_interface
